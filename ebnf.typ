...
#let _error(msg) = text(fill: red, weight: "bold", size: _error-text-size)[⚠ EBNF Error: #msg]
...
#let ebnf(
  ...
  let cells = prods.enumerate().map(((idx, prod)) => {
    ...
    let annot-row(c) = (grid.cell(colspan: 4, if idx > 0 { [#v(production-spacing)#colorize("annot", c)] } else { colorize("annot", c) }),)
    let rows = ()
  ...
  let result = grid(columns: 4, align: (left, center, left, left), column-gutter: column-gap, row-gutter: row-gap, ..cells)
